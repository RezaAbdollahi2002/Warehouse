...
const PrimaryResume = ({ primaryResume, documentationId, setUpdateState, updateState }) => {
  ...
  const resumeHref = useMemo(() => {
    ...
    if (typeof primaryResume === "string" && primaryResume.startsWith("http")) return primaryResume;
  ...
  const add = async (e) => {
    ...
    try {
      ...
      formData.append("primary_resume", primaryResumeFile);

      await api.put("/documentation/change/primary_resume", formData);
  ...
  const remove = async () => {
    try {
      ...
      await api.delete("/documentation/remove/primary_resume");
      toast.success("Primary Resume deleted successfully");
      setUpdateState(!updateState);
    } catch (err) {
      console.error("Error deleting primary resume:", err);
      toast.error("Failed to delete Primary Resume");
  ...
  const view = async () => {
    try {
      ...
      const response = await api.get("/documentation/get/primary_resume", {
        responseType: "blob",
      });
      ...
      toast.success("Primary Resume opened successfully");
    } catch (err) {
      console.error("Error opening primary resume:", err);
      toast.error("Failed to open Primary Resume");
  ...
  const suggestions = async () => {
    try {
      ...
      console.error("Error fetching suggestions for primary resume:", err);
      toast.error("Failed to fetch suggestions for Primary Resume");
